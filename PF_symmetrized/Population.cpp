...
void Population::next_timestep(){
    ...
    std::for_each(begin(moms), end(moms),
    ...
    for (auto& mom : moms){
        ...
        if (mom[0].reproduces() && mom[1].reproduces()) {
            //double dt{ mom[0].damage_transmitted() };
            mom[0].set_damage(mom[0].get_damage()/2.0);
            mom[0].set_resources(mom[0].get_resources() / 2.0);
            //double dt2{ mom[1].damage_transmitted() };
            mom[1].set_damage(mom[1].get_damage()/2.0);
            mom[1].set_resources(mom[1].get_resources() / 2.0);

            Cell c1{ mom[0]};
            Cell c2{ mom[1]};
            c1.mutate();
            c2.mutate();
            gametes.emplace_back(std::move(c1), std::move(c2));
        }
    }
    ...
    for (auto gam : gametes){
        ...
        moms.emplace_back(std::move(gam));
    }
...
void Population::calc_stats(){
    ...
    vd sum = std::transform_reduce(moms.begin(), moms.end(),
...
void Population::write_pop(std::ofstream& file, size_t time){
    for (auto& mom : moms){
